...
def recuperar_conta_cliente(cliente):
    ...
    if len(cliente.contas) > 1:
        ...
        opcao = int(input("Informe o número da conta: "))
...
def transferir(clientes):
    cliente_origem = validar_cliente(clientes)
    ...
    transacao_de_envio = TransferenciaEnviada(valor, conta_origem)
    transacao_de_recebimento = TransferenciaRecebida(valor, conta_destino)

    cliente_origem.realizar_transacao(conta_origem, transacao_de_envio)
    cliente_destino.realizar_transacao(conta_destino, transacao_de_recebimento)

def depositar(clientes):
    cliente = validar_cliente(clientes)
    if not cliente:
        return
